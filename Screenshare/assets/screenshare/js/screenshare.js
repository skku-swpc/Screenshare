...
//		PHONE.HTML
		...
		if ($("#tabs").length)
		{
			...
			$("#menulastli").html(
				'<img src="icon.png" class="icon" style="float: left"/><div style="float: left; text-align: center; padding-left: 5px; font-weight: normal;">Screenshare<br/> <<USERNAME>></div>'+
				'<div style="float: left; text-align: center; padding-left: 5px;">'+
				'<a id="configa" href="config" class="menurighta" target="_top" >`Configuration`</a> <br/>'+
				'<a href="logout" class="menurighta" onclick="try {document.execCommand(\'ClearAuthenticationCache\');} catch (exception) {};" target="_top">`Exit`</a>'+
				'</div>'+
