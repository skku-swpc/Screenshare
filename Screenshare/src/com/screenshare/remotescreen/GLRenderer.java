...
public class GLRenderer extends GLSurfaceView implements Renderer {
	...
	ArrayList<EventsTracking> motionEvents = new ArrayList<EventsTracking>();
	Digest digestTouch;	
	...
	public boolean onTouchEvent(MotionEvent event) {		
		EventsTracking eH = new EventsTracking();	
		...
		if(touchCanBeSent==true){
			if (eH.getAction() == 0 || eH.getAction() == 2)
			{
				...
				ArrayList<EventsTracking> tmpHolder = (ArrayList<EventsTracking>) motionEvents.clone();
				motionEvents.clear();
				sendTouch(tmpHolder);
			}
	...
	public void sendTouch(final ArrayList<EventsTracking> mEvents){
        ...
        new Thread() {
            public void run() {            
				if (mEvents.size() == 0)
					return;
				String data ="";					
				//Log.d("Screenshare_java: ","sendTouch: ("+mEvents.size()+")");
				...
				try{
					...
					//Log.e("Screenshare_java","EXCEPTION VAN: "+e.toString());
					clearTouchBuffer();
				};				
	...
	public void setHttpClient(Digest digestTouch){
	...
	public void getError(GL10 gl){
		...
		while((error = gl.glGetError()) != GL10.GL_NO_ERROR){
			Log.e("Screenshare_java | GLRenderer: ", "glerror: "+error+ " "+GLU.gluErrorString(error));				    	
		}
	}
